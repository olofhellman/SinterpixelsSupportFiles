...
New in SinterPixels 26.3.2, the **record movie frame** command has a **duration** parameter. So, if there is a pause or slowdown desired in an animation -- maybe a frame by frame replay of something, a single movie frame can be made to last longer, rather than needing to call the **record movie frame** command multiple times with the same image.
