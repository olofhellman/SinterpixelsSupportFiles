...
So, it is easy to imagine how to make a star:  set the radius of every other vertex to half its original: 
...
This script uses a shorthand for referring to the five even-numbered vertices.  " vertices {2,4,6,8,10} of p " means the five vertices with indices 2, 4, 6, 8, and 10, so the command. Try the script with different radii or vertex count to get a different star

```
		set the radius of vertices {2,4,6,8,10} of p to 28
```

sends the message to set the radius to each vertex independently.
